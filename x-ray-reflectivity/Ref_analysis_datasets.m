function [R] = Ref_analysis_datasets(RefDataSets,R)
%Function to fit multiple, qzoffset fit data sets with one function
%Author Daniel Kerr 04/06/2016


datasets = fieldnames(RefDataSets);
...
originalFolder = pwd;
number_models = input('Number of protein structures to fit? \n =>');

for modnum = 1:number_models
    model_name{modnum} = input(['Name of protein structure ', num2str(modnum), '? \n =>'], 's');
    rot{modnum} = input('Please input the lower bound, step, and higher bound of theta and phi angles for this protein structure as a consecutive array: \n =>');
    
    [~,PathName{modnum},~]  = uigetfile('*.ed','Select an ed profile in a folder containing all ed profiles for this protein structure');
    %Retrieve ed profile files

    cd(PathName{modnum});
    edcheck = zeros(length(rot{modnum}(1):rot{modnum}(2):rot{modnum}(3)), length(rot{modnum}(4):rot{modnum}(5):rot{modnum}(6)));
    theta_rotcount = 0;
    for theta_rot = rot{modnum}(1):rot{modnum}(2):rot{modnum}(3)
        theta_rotcount = theta_rotcount + 1;
        phi_rotcount = 0;
        for phi_rot = rot{modnum}(4):rot{modnum}(5):rot{modnum}(6)
...
%                         end
    ...
    for theta_rot = rot{modnum}(1):rot{modnum}(2):rot{modnum}(3)
        ...
        for phi_rot = rot{modnum}(4):rot{modnum}(5):rot{modnum}(6)
            ...
            if edcheck(theta_rotcount,phi_rotcount) == 0
                phicount = phicount + 1;
                rotlist.(model_name{modnum}).(['t', num2str(thetacount)]).(['p', num2str(phicount)]) = [theta_rot, phi_rot];
            end
        end
    end
    ...
    if edgen == 1
        ...
        cd(PathName);
        R.(model_name{modnum}).edprof = prot_edpro_func_grid(pdbfile, rotlist, currentFolder, PathName, 0);
        clear rotlist
        theta_rotcount = 0;
        for theta_rot = rot{modnum}(1):rot{modnum}(2):rot{modnum}(3)
            phi_rotcount = 0;
            ...
            for phi_rot = rot{modnum}(4):rot{modnum}(5):rot{modnum}(6)
                phi_rotcount = phi_rotcount + 1;
                rotlist.(model_name{modnum}).(['t', num2str(theta_rotcount)]).(['p', num2str(phi_rotcount)]) = [theta_rot, phi_rot];
            end
        end
...
disp('Fitting will now proceed, each dataset processed will be saved along the way and progress will be displayed below. \n');
...
for modnum = 1:number_models  
    ...
    for datind = 1:length(datasets)
        ...
        RefData = RefDataSets.(datasets{datind});

        filevariable = Ref_analysis_noinput(RefData, model_type, rotlist.(model_name{modnum}), fit_sigma, sigma);  
        
        poolobj = gcp('nocreate');
        delete(poolobj);

        filevariable = chimapplot(filevariable, rot{modnum});

%         if datind < 12
%             R1.(model_name{modnum}).(datasets{datind}) = R.(model_name{modnum}).(datasets{datind});
%         elseif datind > 11 && datind < 20
%             R2.(model_name{modnum}).(datasets{datind}) = R.(model_name{modnum}).(datasets{datind});
%         else
%             R3.(model_name{modnum}).(datasets{datind}) = R.(model_name{modnum}).(datasets{datind});
%         end

        cd(originalFolder); 

        %filevariable = R.(model_name{modnum}).(datasets{datind});
        filename = ['prot2box_', datasets{datind}, '_', model_name{modnum}, '_datafit'];
        save(filename,'filevariable');
        clear filevariable
        
        progress = (floor(modnum)-1)*length(datasets) + datind;
        
        fprintf('Progress = %d/%d\n', progress, number_models*length(datasets));
        
    end
    
    for datind = 1:length(datasets)
        
        temp = load(['prot2box_', (datasets{datind}), '_', (model_name{modnum}), '_datafit.mat']);
        R.(model_name{modnum}).(datasets{datind}) = temp.filevariable;
        R.(model_name{modnum}).(datasets{datind}) = chimapplot(R.(model_name{modnum}).(datasets{datind}), rot{modnum});
        clear temp
            
        AveParams.chi(datind) = R.(model_name{modnum}).(datasets{datind}).chimap.min.chi;
        AveParams.theta(datind) = R.(model_name{modnum}).(datasets{datind}).chimap.min.theta;
        AveParams.phi(datind) = R.(model_name{modnum}).(datasets{datind}).chimap.min.phi;
        AveParams.protpos(datind) = R.(model_name{modnum}).(datasets{datind}).chimap.min.protpos;
        AveParams.cov(datind) = R.(model_name{modnum}).(datasets{datind}).chimap.min.cov;
        AveParams.l_tail(datind) = R.(model_name{modnum}).(datasets{datind}).chimap.min.l_tail;
        AveParams.l_head(datind) = R.(model_name{modnum}).(datasets{datind}).chimap.min.l_head;
        AveParams.p_tail(datind) = R.(model_name{modnum}).(datasets{datind}).chimap.min.p_tail;
        AveParams.p_head(datind) = R.(model_name{modnum}).(datasets{datind}).chimap.min.p_head;
        AveParams.protinsert(datind) = AveParams.l_tail(datind) + AveParams.l_head(datind) + AveParams.protpos(datind);
            
    end
    
    R.(model_name{modnum}).Ave.arrays = AveParams;
    R.(model_name{modnum}).Ave.Params.chi = mean(AveParams.chi);
    R.(model_name{modnum}).Ave.Params.theta = mean(AveParams.theta);
    R.(model_name{modnum}).Ave.Params.phi = mean(AveParams.phi);
    R.(model_name{modnum}).Ave.Params.protpos = mean(AveParams.protpos);
    R.(model_name{modnum}).Ave.Params.pinsert = mean(AveParams.protinsert);
    R.(model_name{modnum}).Ave.Params.cov = mean(AveParams.cov);
    R.(model_name{modnum}).Ave.Params.l_tail = mean(AveParams.l_tail);
    R.(model_name{modnum}).Ave.Params.l_head = mean(AveParams.l_head);
    R.(model_name{modnum}).Ave.Params.p_tail = mean(AveParams.p_tail);
    R.(model_name{modnum}).Ave.Params.p_head = mean(AveParams.p_head);
    R.(model_name{modnum}).Ave.Var.chi = std(AveParams.chi);
    R.(model_name{modnum}).Ave.Var.theta = std(AveParams.theta);
    R.(model_name{modnum}).Ave.Var.phi = std(AveParams.phi);
    R.(model_name{modnum}).Ave.Var.protpos = std(AveParams.protpos);
    R.(model_name{modnum}).Ave.Var.pinsert = std(AveParams.protinsert);
    R.(model_name{modnum}).Ave.Var.cov = std(AveParams.cov);
    R.(model_name{modnum}).Ave.Var.l_tail = std(AveParams.l_tail);
    R.(model_name{modnum}).Ave.Var.l_head = std(AveParams.l_head);
    R.(model_name{modnum}).Ave.Var.p_tail = std(AveParams.p_tail);
    R.(model_name{modnum}).Ave.Var.p_head = std(AveParams.p_head);
    
    AveParamsNames = fieldnames(AveParams);
    
%     for i = 1:length(AveParamsNames)
%         R1.(model_name{modnum}).Ave.arrays.(AveParamsNames{i}) = AveParams.(AveParamsNames{i})(1:11);
%         R2.(model_name{modnum}).Ave.arrays.(AveParamsNames{i}) = AveParams.(AveParamsNames{i})(12:19);
%         R3.(model_name{modnum}).Ave.arrays.(AveParamsNames{i}) = AveParams.(AveParamsNames{i})(20:31);
%     end
        
%     R1.(model_name{modnum}).Ave.Params.chi = mean(AveParams.chi(1:11));
%     R1.(model_name{modnum}).Ave.Params.theta = mean(AveParams.theta(1:11));
%     R1.(model_name{modnum}).Ave.Params.phi = mean(AveParams.phi(1:11));
%     R1.(model_name{modnum}).Ave.Params.protpos = mean(AveParams.protpos(1:11));
%     R1.(model_name{modnum}).Ave.Params.pinsert = mean(AveParams.protinsert(1:11));
%     R1.(model_name{modnum}).Ave.Params.cov = mean(AveParams.cov(1:11));
%     R1.(model_name{modnum}).Ave.Params.l_tail = mean(AveParams.l_tail(1:11));
%     R1.(model_name{modnum}).Ave.Params.l_head = mean(AveParams.l_head(1:11));
%     R1.(model_name{modnum}).Ave.Params.p_tail = mean(AveParams.p_tail(1:11));
%     R1.(model_name{modnum}).Ave.Params.p_head = mean(AveParams.p_head(1:11));
%     R1.(model_name{modnum}).Ave.Var.chi = std(AveParams.chi(1:11));
%     R1.(model_name{modnum}).Ave.Var.theta = std(AveParams.theta(1:11));
%     R1.(model_name{modnum}).Ave.Var.phi = std(AveParams.phi(1:11));
%     R1.(model_name{modnum}).Ave.Var.protpos = std(AveParams.protpos(1:11));
%     R1.(model_name{modnum}).Ave.Var.pinsert = std(AveParams.protinsert(1:11));
%     R1.(model_name{modnum}).Ave.Var.cov = std(AveParams.cov(1:11));
%     R1.(model_name{modnum}).Ave.Var.l_tail = std(AveParams.l_tail(1:11));
%     R1.(model_name{modnum}).Ave.Var.l_head = std(AveParams.l_head(1:11));
%     R1.(model_name{modnum}).Ave.Var.p_tail = std(AveParams.p_tail(1:11));
%     R1.(model_name{modnum}).Ave.Var.p_head = std(AveParams.p_head(1:11));
%     
%     R2.(model_name{modnum}).Ave.Params.chi = mean(AveParams.chi(12:19));
%     R2.(model_name{modnum}).Ave.Params.theta = mean(AveParams.theta(12:19));
%     R2.(model_name{modnum}).Ave.Params.phi = mean(AveParams.phi(12:19));
%     R2.(model_name{modnum}).Ave.Params.protpos = mean(AveParams.protpos(12:19));
%     R2.(model_name{modnum}).Ave.Params.pinsert = mean(AveParams.protinsert(12:19));
%     R2.(model_name{modnum}).Ave.Params.cov = mean(AveParams.cov(12:19));
%     R2.(model_name{modnum}).Ave.Params.l_tail = mean(AveParams.l_tail(12:19));
%     R2.(model_name{modnum}).Ave.Params.l_head = mean(AveParams.l_head(12:19));
%     R2.(model_name{modnum}).Ave.Params.p_tail = mean(AveParams.p_tail(12:19));
%     R2.(model_name{modnum}).Ave.Params.p_head = mean(AveParams.p_head(12:19));
%     R2.(model_name{modnum}).Ave.Var.chi = std(AveParams.chi(12:19));
%     R2.(model_name{modnum}).Ave.Var.theta = std(AveParams.theta(12:19));
%     R2.(model_name{modnum}).Ave.Var.phi = std(AveParams.phi(12:19));
%     R2.(model_name{modnum}).Ave.Var.protpos = std(AveParams.protpos(12:19));
%     R2.(model_name{modnum}).Ave.Var.pinsert = std(AveParams.protinsert(12:19));
%     R2.(model_name{modnum}).Ave.Var.cov = std(AveParams.cov(12:19));
%     R2.(model_name{modnum}).Ave.Var.l_tail = std(AveParams.l_tail(12:19));
%     R2.(model_name{modnum}).Ave.Var.l_head = std(AveParams.l_head(12:19));
%     R2.(model_name{modnum}).Ave.Var.p_tail = std(AveParams.p_tail(12:19));
%     R2.(model_name{modnum}).Ave.Var.p_head = std(AveParams.p_head(12:19));
%     
%     R3.(model_name{modnum}).Ave.Params.chi = mean(AveParams.chi(20:31));
%     R3.(model_name{modnum}).Ave.Params.theta = mean(AveParams.theta(20:31));
%     R3.(model_name{modnum}).Ave.Params.phi = mean(AveParams.phi(20:31));
%     R3.(model_name{modnum}).Ave.Params.protpos = mean(AveParams.protpos(20:31));
%     R3.(model_name{modnum}).Ave.Params.pinsert = mean(AveParams.protinsert(20:31));
%     R3.(model_name{modnum}).Ave.Params.cov = mean(AveParams.cov(20:31));
%     R3.(model_name{modnum}).Ave.Params.l_tail = mean(AveParams.l_tail(20:31));
%     R3.(model_name{modnum}).Ave.Params.l_head = mean(AveParams.l_head(20:31));
%     R3.(model_name{modnum}).Ave.Params.p_tail = mean(AveParams.p_tail(20:31));
%     R3.(model_name{modnum}).Ave.Params.p_head = mean(AveParams.p_head(20:31));
%     R3.(model_name{modnum}).Ave.Var.chi = std(AveParams.chi(20:31));
%     R3.(model_name{modnum}).Ave.Var.theta = std(AveParams.theta(20:31));
%     R3.(model_name{modnum}).Ave.Var.phi = std(AveParams.phi(20:31));
%     R3.(model_name{modnum}).Ave.Var.protpos = std(AveParams.protpos(20:31));
%     R3.(model_name{modnum}).Ave.Var.pinsert = std(AveParams.protinsert(20:31));
%     R3.(model_name{modnum}).Ave.Var.cov = std(AveParams.cov(20:31));
%     R3.(model_name{modnum}).Ave.Var.l_tail = std(AveParams.l_tail(20:31));
%     R3.(model_name{modnum}).Ave.Var.l_head = std(AveParams.l_head(20:31));
%     R3.(model_name{modnum}).Ave.Var.p_tail = std(AveParams.p_tail(20:31));
%     R3.(model_name{modnum}).Ave.Var.p_head = std(AveParams.p_head(20:31));
    
end

end

function [R] = Ref_analysis_noinput(R, model_type, rotlist, fit_sigma, sigma)
%Master program for complete reduction and error analysis of x-ray
%reflectivity data.

%model_type = 1 lipid only, model_name should be an empty array
%model_type = 2 lipid+protein
%rotlist should be a structure file containing the list of orientations to
%fit, if lipid only then it should be an empty array
%If fit_sigma = 0, sigma won't be fit and sigma should be provided, if
%fit_sigma = 1, then use an empty array as input for sigma
